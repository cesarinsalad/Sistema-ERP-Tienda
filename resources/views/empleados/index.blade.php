            ...
            <div class="card border-0 shadow-sm mb-4" style="border-radius: 1.25rem;">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div>
                            <h3 class="font-weight-bold text-dark m-0" style="letter-spacing: -0.5px;">Lista de Empleados</h3>
                            <p class="text-muted small m-0 mt-1"><i class="fas fa-users mr-1"></i> Gestión de personal y perfiles administrativos</p>
                        </div>
                        <div class="d-flex" style="gap: 12px;">
                            <a class="btn-premium-return" href="{{ route('empleados.inactivos') }}">
                                <i class="fas fa-archive"></i> VER INACTIVOS
                            </a>
                            <a class="btn px-4 py-2 font-weight-bold shadow-sm" href="{{ route('empleados.create') }}" 
                    ...
                    <div class="mt-4 pt-3 border-top">
                        <form action="{{ route('empleados.index') }}" method="GET" class="form-inline">
                            ...
                            @if(request('search'))
                                <a href="{{ route('empleados.index') }}" class="btn btn-link text-muted ml-2 font-weight-bold">Limpiar filtro</a>
                            @endif
                        </form>
                    </div>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td style="min-width: 180px;">
                                        <div class="d-flex align-items-center">
                                            <span class="font-weight-bold text-dark">{{ $empleado->user->name ?? 'N/A' }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <span class="badge {{ $empleado->is_active ? 'badge-success' : 'badge-danger' }} px-2 py-1" style="border-radius: 6px; font-size: 0.75rem;">
                                            {{ $empleado->is_active ? 'ACTIVO' : 'INACTIVO' }}
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center" style="gap: 8px; white-space: nowrap;">
                                            <a class="btn btn-sm btn-info shadow-sm" href="{{ route('empleados.show', $empleado->id) }}" 
                                            ...
                                            <form action="{{ route('empleados.destroy', $empleado->id) }}" method="POST" class="m-0">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm {{ $empleado->is_active ? 'btn-danger' : 'btn-success' }} shadow-sm"
                                                        style="border-radius: 8px; width: 32px; height: 32px; display: flex; align-items: center; justify-content: center;"
                                                        data-toggle="tooltip" title="{{ $empleado->is_active ? 'Desactivar' : 'Reactivar' }}">
